...
from 
(
	select * from @resultsSchema.dbo.heracles_results where analysis_id = 109 and cohort_definition_id in (@cohortDefinitionId)
) hr1,
(
	select min(cast(stratum_1 as int)) as MinValue 
	from @resultsSchema.dbo.heracles_results where analysis_id = 109 and cohort_definition_id in (@cohortDefinitionId)
) MinValue,
(
