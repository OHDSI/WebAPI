...
FROM (
	SELECT ROW_NUMBER() OVER (ORDER BY e.cohort_start_date) AS id,
	  ...
	  e.cohort_start_date,
	  e.cohort_end_date
	FROM @temp_database_schema.@target_cohort_table e
	  JOIN ( @event_cohort_id_index_map ) ec ON e.cohort_definition_id = ec.cohort_definition_id
	  JOIN @temp_database_schema.@target_cohort_table t ON t.cohort_start_date <= e.cohort_start_date AND e.cohort_end_date <= t.cohort_end_date AND t.subject_id = e.subject_id
...
FROM (
  SELECT COUNT(*) cnt
  FROM @temp_database_schema.@target_cohort_table
