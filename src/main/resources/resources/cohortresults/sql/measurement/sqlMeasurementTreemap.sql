...
from (select * from @resultsSchema.heracles_results where analysis_id = 1300 and cohort_definition_id in (@cohortDefinitionId)) hr1
	inner join
	(select * from @resultsSchema.heracles_results where analysis_id = 1301 and cohort_definition_id in (@cohortDefinitionId)) hr2
	...
	(
		...
		) obs left join @cdmSchema.concept_ancestor ca1 on obs.concept_id = ca1.DESCENDANT_CONCEPT_ID and ca1.min_levels_of_separation = 1
		left join @cdmSchema.concept c1 on ca1.ANCESTOR_CONCEPT_ID = c1.concept_id
		left join @cdmSchema.concept_ancestor ca2 on c1.concept_id = ca2.DESCENDANT_CONCEPT_ID and ca2.min_levels_of_separation = 1
		left join @cdmSchema.concept c2 on ca2.ANCESTOR_CONCEPT_ID = c2.concept_id
		left join @cdmSchema.concept_ancestor ca3 on c2.concept_id = ca3.DESCENDANT_CONCEPT_ID and ca3.min_levels_of_separation = 1
		left join @cdmSchema.concept c3 on ca3.ANCESTOR_CONCEPT_ID = c3.concept_id
