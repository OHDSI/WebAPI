...
public class PageableValueFactoryProvider implements ValueFactoryProvider {
    ...
    private static class PageableValueFactory
        ...
        public Pageable provide() {
            ...
            if (StringUtils.isNotEmpty(facets) || StringUtils.isNotEmpty(text)) {
                final Map<String, List<String>> f;
                if(facets != null) {
                    f = new HashMap<>();
                    Arrays.stream(facets.split("&")).filter(StringUtils::isNotEmpty).forEach(s -> {
                        final String[] keyValue = s.split("=");
                        assert keyValue.length == 2;
                        f.computeIfAbsent(keyValue[0], list -> new ArrayList<>()).addAll(Arrays.asList(keyValue[1].split(",")));
                    });
                } else {
                    f = null;
                }
                return new FilteredPageRequest(page, size, orders.isEmpty() ? null : new Sort(orders), new Filter(text, f));
            } else {
