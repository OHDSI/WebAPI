...
INSERT INTO ${ohdsiSchema}.sec_permission (id, value, description, for_role_id)
  SELECT ${ohdsiSchema}.sec_permission_id_seq.nextval, REPLACE(CAST(new_perms.val AS VARCHAR2(255)), '%s', REPLACE(REPLACE(value, 'cohortdefinition:*:generate:', ''), ':get', '')),
    REPLACE(CAST(new_perms.description AS VARCHAR2(255)), '%s', REPLACE(REPLACE(value, 'cohortdefinition:*:generate:', ''), ':get', '')), role_id
  FROM ${ohdsiSchema}.sec_permission sp
    JOIN ${ohdsiSchema}.sec_role_permission srp on sp.id = srp.permission_id
    CROSS JOIN (
                 SELECT 'cohortdefinition:%s:export:conceptset:get' val, 'Export ConceptSet for Source with key=%s' description FROM DUAL
               ) new_perms
