SELECT
  ...
  SUM(ROUND(1000 * (1.0 * num_count_value / denom_count_value), 5)) AS y_prevalence_1000_pp --prevalence, per 1000 persons
FROM (
       SELECT
...
WHERE c1.concept_id in (@conceptIds)
GROUP BY c2.concept_name, num_stratum_2, num_stratum_4
