select N.cohort_definition_id
  ...
  , ((N.avg_value * 1.0)*N.count_value)/365.25 as exposure_years_total
  , (((N.avg_value * 1.0)*N.count_value))/(((D.avg_value * 1.0)*D.count_value)) * 100 as exposure_percent
  , (N.avg_value * 1000.0)/365.25 as exposure_avg_years_1k
from (
	select cohort_definition_id, analysis_id, stratum_1, count_value, avg_value
	from @results_schema.heracles_results_dist where stratum_1 != ''
) N
inner join (
	select cohort_definition_id, analysis_id, stratum_1, count_value, avg_value from @results_schema.heracles_results_dist where stratum_1 = ''
