...
FROM @evidenceSchema.NC_LU_CONCEPT_UNIVERSE u
	JOIN @evidenceSchema.CONCEPT_ANCESTOR ca1
		ON u.DRUG_CONCEPT_ID = ca1.ANCESTOR_CONCEPT_ID
	JOIN @evidenceSchema.CONCEPT_ANCESTOR ca2
		ON u.CONDITION_CONCEPT_ID = ca2.ANCESTOR_CONCEPT_ID
	JOIN @evidenceSchema.CEM_UNIFIED u2
		ON u2.CONCEPT_ID_1 = ca1.DESCENDANT_CONCEPT_ID 
                AND u2.CONCEPT_ID_2 = ca2.DESCENDANT_CONCEPT_ID 
                {@targetDomain=='CONDITION'}?{AND u2.CONCEPT_ID_1 != ca1.ANCESTOR_CONCEPT_ID}
                {@targetDomain=='DRUG'}?{AND u2.CONCEPT_ID_2 != ca2.ANCESTOR_CONCEPT_ID}
...
AND ca1.ANCESTOR_CONCEPT_ID IN (@drugList)
AND ca2.ANCESTOR_CONCEPT_ID IN (@conditionList)
...
JOIN @vocabularySchema.CONCEPT c2
	ON c2.CONCEPT_ID = u2.CONCEPT_ID_1
WHERE u2.CONCEPT_ID_1 IN (@drugList)
...
FROM @evidenceSchema.NC_LU_CONCEPT_UNIVERSE u
	...
	JOIN @vocabularySchema.CONCEPT c2
		ON c2.CONCEPT_ID = u2.CONCEPT_ID_1
WHERE cr.CONCEPT_ID_1 IN (@conditionList)
...
FROM @evidenceSchema.NC_LU_CONCEPT_UNIVERSE u
	...
	JOIN @vocabularySchema.CONCEPT c2
		ON c2.CONCEPT_ID = u2.CONCEPT_ID_1
WHERE ca.DESCENDANT_CONCEPT_ID IN (@conditionList)
AND u2.CONCEPT_ID_1 IN (@drugList)
AND u2.CONCEPT_ID_2 != 0
AND u2.CONCEPT_ID_1 != 0
;