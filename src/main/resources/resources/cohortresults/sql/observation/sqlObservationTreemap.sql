...
from (select * from @resultsSchema.dbo.heracles_results where analysis_id = 800 and cohort_definition_id in (@cohortDefinitionId)) hr1
	inner join
	(select * from @resultsSchema.dbo.heracles_results where analysis_id = 801 and cohort_definition_id in (@cohortDefinitionId)) hr2
