...
if [ "$1" = 'run-webapi' ]; then
  ...
  if [[ ! -z "$FEDER8_WEBAPI_SECURE" && "$FEDER8_WEBAPI_SECURE" == "true" ]]; then
    ...
    if [[ ! -z "$FEDER8_WEBAPI_CENTRAL" && "$FEDER8_WEBAPI_CENTRAL" == "true" ]]; then
      ...
      JAVA_OPTS="${JAVA_OPTS} -Dsecurity.ldap.dn=${FEDER8_WEBAPI_LDAP_DN} -Dsecurity.ldap.url=${FEDER8_WEBAPI_LDAP_URL} -Dsecurity.ldap.baseDn=${FEDER8_WEBAPI_LDAP_BASEDN} -Dsecurity.ldap.searchString=(&(objectClass=person)(${FEDER8_WEBAPI_LDAP_DN})) -Dsecurity.ldap.userMapping.usernameAttr=${FEDER8_WEBAPI_LDAP_USERNAME_ATTRIBUTE} -Dsecurity.ldap.searchBase=${FEDER8_WEBAPI_LDAP_BASEDN} -Dsecurity.ldap.system.username=${FEDER8_WEBAPI_LDAP_SYSTEM_USERNAME} -Dsecurity.ldap.system.password=${FEDER8_WEBAPI_LDAP_SYSTEM_PASSWORD}"
    fi
  fi
