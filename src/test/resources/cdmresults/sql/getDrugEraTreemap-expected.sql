...
from (select * from result_schema.achilles_results where analysis_id = 900 ) hr1
	inner join
	(select stratum_1, avg_value from result_schema.achilles_results_dist where analysis_id = 907 ) hr2
	...
	(
  	select rxnorm.rxnorm_ingredient_concept_id,
		...
		left join
			...
			from
			vocab_schema.concept c1
			inner join 
			vocab_schema.concept_ancestor ca1
		...
		left join vocab_schema.concept atc1
