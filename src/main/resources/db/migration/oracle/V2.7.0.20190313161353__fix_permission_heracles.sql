...
SELECT ${ohdsiSchema}.sec_permission_id_seq.nextval, REPLACE(new_perms.val, '%s', REPLACE(REPLACE(value, 'source:', ''), ':access', '')), REPLACE(new_perms.descr, '%s', REPLACE(REPLACE(value, 'source:', ''), ':access', '')), role_id
  ...
  CROSS JOIN (
    SELECT 'cohortresults:%s:*:healthcareutilization:*:*:get' val, 'Get cohort results baseline on period for Source with SourceKey = %s' descr FROM DUAL
    UNION ALL
    SELECT 'cohortresults:%s:*:healthcareutilization:*:*:*:get', 'Get cohort results baseline on occurrence for Source with SourceKey = %s' FROM DUAL
  ) new_perms
