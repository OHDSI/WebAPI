...
3001 as analysis_id,
row_number() over (
partition by cast((cast(year(c.cohort_start_date) - p.year_of_birth as int)) / 10 * 10 as int), p.gender_concept_id
...
WITH cteRawData as
(
	select cohort_definition_id, gender_concept_id AS stratum_1, count_value FROM #raw_106
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, visit_concept_id as stratum_1, count_value FROM #raw_211
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, condition_concept_id as stratum_1, gender_concept_id as stratum_2, count_value FROM #raw_406
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, gender_concept_id as stratum_1, count_value FROM #raw_506
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, drug_concept_id AS stratum_1, gender_concept_id AS stratum_2, count_value FROM #raw_706
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, drug_concept_id AS stratum_1, count_value FROM #raw_715
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, drug_concept_id AS stratum_1, count_value FROM #raw_716
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, observation_concept_id AS stratum_1, unit_concept_id AS stratum_2, count_value FROM #raw_815
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, drug_concept_id AS stratum_1, gender_concept_id AS stratum_2, count_value FROM #raw_906
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, drug_concept_id AS stratum_1, count_value FROM #raw_907
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, condition_concept_id AS stratum_1, count_value FROM #raw_1007
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, measurement_concept_id as stratum_1, unit_concept_id as stratum_2, count_value FROM #raw_1316
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, measurement_concept_id as stratum_1, unit_concept_id as stratum_2, count_value FROM #raw_1317
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, gender_concept_id as stratum_1, count_value FROM #raw_1802
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, cohort_year as stratum_1, count_value FROM #raw_1803
), 
overallStats as
(
  select cohort_definition_id, 
...
WITH cteRawData as
(
	select cohort_definition_id, gender_concept_id as stratum_1, count_value FROM #raw_1809
), 
overallStats as
(
  select cohort_definition_id, 
...
select c1.cohort_definition_id,
	c1.subject_id,
	CAST(floor((year(c1.cohort_start_date) - p1.YEAR_OF_BIRTH)/10) AS INT) as age_decile,
	DATEDIFF(dd,c1.cohort_start_date, c1.cohort_end_date) as count_value
INTO #raw_1810
from @CDM_schema.person p1
inner join #HERACLES_cohort c1 on p1.person_id = c1.subject_id
...
WITH cteRawData as
(
	select cohort_definition_id, age_decile as stratum_1, count_value FROM #raw_1810
), 
overallStats as
(
  select cohort_definition_id, 
...
(
	select distinct c1.cohort_definition_id as cohort_definition_id,
		c1.subject_id as subject_id,
		de1.drug_concept_id as drug_concept_id,
		de1.drug_type_concept_id as drug_type_concept_id,
