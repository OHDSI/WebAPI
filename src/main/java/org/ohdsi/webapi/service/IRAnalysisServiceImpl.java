 ...
 */
@Component
public class IRAnalysisServiceImpl extends AbstractDaoService implements GeneratesNotification, IRAnalysisService {

  private static final Logger log = LoggerFactory.getLogger(IRAnalysisServiceImpl.class);
  ...
  @Override
  public List<IRAnalysisServiceImpl.IRAnalysisListItem> getIRAnalysisList() {
    ...
    return getTransactionTemplate().execute(transactionStatus -> {
      ...
      return StreamSupport.stream(analysisList.spliterator(), false).map(p -> {
        IRAnalysisServiceImpl.IRAnalysisListItem item = new IRAnalysisServiceImpl.IRAnalysisListItem();
        item.id = p.getId();
        item.name = p.getName();
