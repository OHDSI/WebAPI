select cohort_definition_id, analysis_id,
  ...
  cast(median_value as float), cast(p10_value as float), cast(p25_value as float), cast(p75_value as float), cast(p90_value as float) from #results_dist_@analysisId
