...
from (select * from @resultsSchema.dbo.heracles_results where analysis_id = 1300 and cohort_definition_id in (@cohortDefinitionId)) hr1
	inner join
	(select * from @resultsSchema.dbo.heracles_results where analysis_id = 1301 and cohort_definition_id in (@cohortDefinitionId)) hr2
