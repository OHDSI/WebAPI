...
from (select * from @resultsSchema.heracles_results where analysis_id = 3 and cohort_definition_id in (@cohortDefinitionId)) hr1,
	(select min(cast(stratum_1 as int)) as MinValue from @resultsSchema.heracles_results where analysis_id = 3 and cohort_definition_id in (@cohortDefinitionId)) MinValue,
