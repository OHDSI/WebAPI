...
public class CohortSampleMvcController extends AbstractMvcController {
    ...
    @Autowired
    public CohortSampleMvcController(
     ...
     */
    @GetMapping("/{cohortDefinitionId}/{sourceKey}")
    public ResponseEntity<CohortSampleListDTO> listCohortSamples(
     ...
     */
    @GetMapping("/{cohortDefinitionId}/{sourceKey}/{sampleId}")
    public ResponseEntity<CohortSampleDTO> getCohortSample(
            @PathVariable("cohortDefinitionId") int cohortDefinitionId,
            @PathVariable("sourceKey") String sourceKey,
        ...
        return ok(this.samplingService.getSample(sampleId, withRecordCounts));
    }
     ...
     */
    @PostMapping("/{cohortDefinitionId}/{sourceKey}/{sampleId}/refresh")
    public ResponseEntity<CohortSampleDTO> refreshCohortSample(
            @PathVariable("cohortDefinitionId") int cohortDefinitionId,
            @PathVariable("sourceKey") String sourceKey,
            @PathVariable("sampleId") Integer sampleId,
            @RequestParam(defaultValue = "") String fields
     ...
     */
    @GetMapping("/has-samples/{cohortDefinitionId}")
    public ResponseEntity<Map<String, Boolean>> hasSamples(
            @PathVariable("cohortDefinitionId") int cohortDefinitionId
    ) {
        int nSamples = this.samplingService.countSamples(cohortDefinitionId);
        return ok(Collections.singletonMap("hasSamples", nSamples > 0));
    }
     ...
     * @return true or false
     */
    @GetMapping("/has-samples/{cohortDefinitionId}/{sourceKey}")
    public ResponseEntity<Map<String, Boolean>> hasSamplesForSource(
            @PathVariable("sourceKey") String sourceKey,
            @PathVariable("cohortDefinitionId") int cohortDefinitionId
    ) {
        Source source = getSource(sourceKey);
     ...
     * @return
     */
    @PostMapping("/{cohortDefinitionId}/{sourceKey}")
    public ResponseEntity<CohortSampleDTO> createCohortSample(
            @PathVariable("sourceKey") String sourceKey,
            @PathVariable("cohortDefinitionId") int cohortDefinitionId,
        ...
        return ok(samplingService.createSample(source, cohortDefinitionId, sampleParameters));
    }
