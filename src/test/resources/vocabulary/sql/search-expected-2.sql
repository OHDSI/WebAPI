...
where 1=1 AND DOMAIN_ID IN (?,?,?,?,?) AND VOCABULARY_ID IN (?,?,?,?,?,?,?,?,?,?) AND CONCEPT_CLASS_ID IN (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) AND INVALID_REASON IS NULL  AND STANDARD_CONCEPT = ?
AND (LOWER(CONCEPT_NAME) LIKE ? or LOWER(CONCEPT_CODE) LIKE ?)
