...
from (select * from @ohdsi_database_schema.heracles_results where analysis_id = 200 and cohort_definition_id = @cohortDefinitionId) hr1
	...
	inner join @cdm_database_schema.concept c1 on CAST(CASE WHEN isNumeric(hr1.stratum_1) = 1 THEN hr1.stratum_1 ELSE null END AS INT) = c1.concept_id as VARCHAR(255),
