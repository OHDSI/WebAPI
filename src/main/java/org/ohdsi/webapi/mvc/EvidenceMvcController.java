...
public class EvidenceMvcController extends AbstractMvcController implements GeneratesNotification {
    ...
    @Autowired
    private EvidenceService daoService;
     ...
     */
    @GetMapping("/study/{cohortId}")
    public ResponseEntity<Collection<CohortStudyMapping>> getCohortStudyMapping(@PathVariable("cohortId") int cohortId) {
        return ok(cohortStudyMappingRepository.findByCohortDefinitionId(cohortId));
    }
     ...
     */
    @GetMapping("/mapping/{conceptId}")
    public ResponseEntity<Collection<ConceptCohortMapping>> getConceptCohortMapping(@PathVariable("conceptId") int conceptId) {
        return ok(mappingRepository.findByConceptId(conceptId));
    }
     ...
     */
    @GetMapping("/conceptofinterest/{conceptId}")
    public ResponseEntity<Collection<ConceptOfInterestMapping>> getConceptOfInterest(@PathVariable("conceptId") int conceptId) {
        return ok(conceptOfInterestMappingRepository.findAllByConceptId(conceptId));
    }
     ...
     */
    @GetMapping("/label/{setid}")
    public ResponseEntity<Collection<DrugLabel>> getDrugLabel(@PathVariable("setid") String setid) {
        return ok(drugLabelRepository.findAllBySetid(setid));
    }
     ...
     */
    @GetMapping("/labelsearch/{searchTerm}")
    public ResponseEntity<Collection<DrugLabel>> searchDrugLabels(@PathVariable("searchTerm") String searchTerm) {
        return ok(drugLabelRepository.searchNameContainsTerm(searchTerm));
    }
     ...
     */
    @GetMapping("/{sourceKey}/info")
    public ResponseEntity<Collection<EvidenceInfo>> getInfo(@PathVariable("sourceKey") String sourceKey) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        ...
        return ok(daoService.getSourceJdbcTemplate(source).query(psr.getSql(), psr.getSetter(), (rs, rowNum) -> {
     ...
     * @return
     */
    @PostMapping("/{sourceKey}/drugconditionpairs")
    public ResponseEntity<Collection<DrugHoiEvidence>> getDrugConditionPairs(@PathVariable("sourceKey") String sourceKey, @RequestBody DrugConditionSourceSearchParams searchParams) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        String sql = getDrugHoiEvidenceSQL(source, searchParams);
        return ok(daoService.getSourceJdbcTemplate(source).query(sql, (rs, rowNum) -> {
     ...
     */
    @GetMapping("/{sourceKey}/drug/{id}")
    public ResponseEntity<Collection<DrugEvidence>> getDrugEvidence(@PathVariable("sourceKey") String sourceKey, @PathVariable("id") final Long id) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
     ...
     * @return A list of evidence
     */
    @GetMapping("/{sourceKey}/hoi/{id}")
    public ResponseEntity<Collection<HoiEvidence>> getHoiEvidence(@PathVariable("sourceKey") String sourceKey, @PathVariable("id") final Long id) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        PreparedStatementRenderer psr = prepareGetEvidenceForConcept(source, id);
        return ok(daoService.getSourceJdbcTemplate(source).query(psr.getSql(), psr.getSetter(), (rs, rowNum) -> {
            String evidenceSource = rs.getString("SOURCE_ID");
     ...
     */
    @PostMapping("/{sourceKey}/druglabel")
    public ResponseEntity<Collection<DrugLabelInfo>> getDrugIngredientLabel(@PathVariable("sourceKey") String sourceKey, @RequestBody long[] identifiers) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        return ok(executeGetDrugLabels(identifiers, source));
    }
     ...
     * @return A list of evidence for the drug and HOI
     */
    @GetMapping("/{sourceKey}/drughoi/{key}")
    public ResponseEntity<List<DrugHoiEvidence>> getDrugHoiEvidence(@PathVariable("sourceKey") String sourceKey, @PathVariable("key") final String key) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
     ...
     */
    @GetMapping("/{sourceKey}/drugrollup/{filter}/{id}")
     ...
     */
    @GetMapping("/{sourceKey}/{id}")
    public ResponseEntity<Collection<Evidence>> getEvidence(@PathVariable("sourceKey") String sourceKey, @PathVariable("id") final Long id) {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        PreparedStatementRenderer psr = prepareGetEvidenceForConcept(source, id);
        return ok(daoService.getSourceJdbcTemplate(source).query(psr.getSql(), psr.getSetter(), (rs, rowNum) -> {
            String evidenceSource = rs.getString("SOURCE_ID");
     ...
     */
    @GetMapping("/{sourceKey}/evidencesummary")
     ...
     */
    @GetMapping("/{sourceKey}/evidencedetails")
     ...
     * @throws IOException
     */
    @PostMapping("/{sourceKey}/spontaneousreports")
     ...
     * @throws IOException
     */
    @PostMapping("/{sourceKey}/evidencesearch")
     ...
     * @throws IOException
     */
    @PostMapping("/{sourceKey}/labelevidence")
     ...
     * @throws Exception
     */
    @PostMapping("/{sourceKey}/negativecontrols")
    public ResponseEntity<JobExecutionResource> queueNegativeControlsJob(@PathVariable("sourceKey") String sourceKey, @RequestBody NegativeControlTaskParameters task) throws Exception {
        if (task == null) {
            return ok(null);
        }
        ...
        String ohdsiDatasourceSourceDialect = daoService.getSourceDialect();
        task.setSourceDialect(ohdsiDatasourceSourceDialect);
        task.setOhdsiSchema(daoService.getOhdsiSchema());
        ...
        if (cemSchema == null) {
            throw new RuntimeException("Evidence daimon not configured for source.");
        }
        if (cemResultsSchema == null) {
            throw new RuntimeException("Results daimon not configured for source.");
        }
        ...
        if (task.getCsToInclude() > 0) {
            try {
                ...
                // log warning would go here if logger was available
            }
        }
        ...
        NegativeControlTasklet tasklet = new NegativeControlTasklet(task, daoService.getSourceJdbcTemplate(task.getSource()), task.getJdbcTemplate(),
                daoService.getTransactionTemplate(), this.conceptSetGenerationInfoRepository, daoService.getSourceDialect());

        return ok(this.jobTemplate.launchTasklet(NAME, "negativeControlsAnalysisStep", tasklet, jobParameters));
    }
     ...
     */
    @GetMapping("/{sourceKey}/negativecontrols/{conceptsetid}")
    public ResponseEntity<Collection<NegativeControlDTO>> getNegativeControls(@PathVariable("sourceKey") String sourceKey, @PathVariable("conceptsetid") int conceptSetId) throws Exception {
        Source source = daoService.getSourceRepository().findBySourceKey(sourceKey);
        PreparedStatementRenderer psr = this.prepareGetNegativeControls(source, conceptSetId);
        final List<NegativeControlDTO> recs = daoService.getSourceJdbcTemplate(source).query(psr.getSql(), psr.getSetter(), new NegativeControlMapper());
        return ok(recs);
    }
    ...
    public ResponseEntity<String> getNegativeControlsSqlStatement(@PathVariable("sourceKey") String sourceKey,
        ...
        return ok(getNegativeControlSql(task));
    }
    ...
    public static String getNegativeControlSql(NegativeControlTaskParameters task) {
        ...
        sb.append("-- ").append(sqlFile).append("\n\n");
        sql = getJobResultsDeleteStatementSql(cemResultsSchema, task.getConceptSetId());
        sb.append(sql + "\n\n");
    ...
    protected Collection<DrugLabelInfo> executeGetDrugLabels(long[] identifiers, Source source) {
        ...
        if (identifiers.length == 0) {
            ...
            info.addAll(daoService.getSourceJdbcTemplate(source).query(psr.getSql(), psr.getSetter(), this.drugLabelRowMapper));
            return info;
        }
    }
