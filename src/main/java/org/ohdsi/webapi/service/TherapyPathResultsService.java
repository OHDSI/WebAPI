...
public class TherapyPathResultsService extends AbstractDaoService {
  ...
  public List<TherapyPathReport> getReports() {
    try {
      String sql_statement = ResourceHelper.GetResourceAsString("/resources/therapypathresults/sql/getTherapyPathReports.sql");
      sql_statement = SqlRender.renderSql(sql_statement, new String[]{"OHDSI_schema"}, new String[]{getOhdsiSchema()});
      sql_statement = SqlTranslate.translateSql(sql_statement, "sql server", getDialect());
      return getJdbcTemplate().query(sql_statement, new RowMapper<TherapyPathReport>() {
        @Override
  ...
  public List<TherapyPathVector> getTherapyPathVectors(@PathParam("id") String id) {
    try {
      ...
      sql_statement = SqlTranslate.translateSql(sql_statement, "sql server", getDialect());
      return getJdbcTemplate().query(sql_statement, new RowMapper<TherapyPathVector>() {
        @Override
  ...
  public List<TherapySummary> getSummaries(String[] identifiers) {
    try {
      ...
      sql_statement = SqlTranslate.translateSql(sql_statement, "sql server", getDialect());
      return getJdbcTemplate().query(sql_statement, new RowMapper<TherapySummary>() {
        @Override
