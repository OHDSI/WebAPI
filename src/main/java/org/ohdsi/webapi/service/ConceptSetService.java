...
public class ConceptSetService extends AbstractDaoService {
    ...
    public Response exportConceptSetList(int[] conceptSetIds) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < conceptSetIds.length; i++) {
                // Get the concept set information
                cs.add(getConceptSetForExport(conceptSetIds[i], sourceInfo));
            }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public Response exportConceptSetToCSV(@PathParam("id") final int id) throws Exception {
        return this.exportConceptSetList(new int[] { id });
    }
